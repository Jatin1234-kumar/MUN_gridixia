...
export default function CountryAllocation() {
  ...
  const committeeName = allocatedCommittee?.name || draft?.committeePreference.preferredCommitteeName || 'Awaiting assignment';
  const agenda = allocatedCommittee?.topic || allocatedEvent?.description || 'Committee agenda will appear once allocation is confirmed.';
        ...
        <Card className="glass-card overflow-hidden border-white/[0.08]">
          <CardHeader className="border-b border-white/[0.06] bg-white/[0.015]">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <CardDescription className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Delegate Portfolio</CardDescription>
                <CardTitle className="mt-2 text-2xl text-foreground">Country Allocation</CardTitle>
                <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                  Review your diplomatic assignment in a premium card designed for quick operational scanning.
                </p>
              </div>
              <div className={cn('rounded-2xl border px-4 py-3 text-right', isPending ? 'border-amber-500/20 bg-amber-500/10' : 'border-emerald-500/20 bg-emerald-500/10')}>
                <p className={cn('text-[10px] uppercase tracking-[0.3em]', isPending ? 'text-amber-300' : 'text-emerald-300')}>Allocation State</p>
                <p className={cn('mt-1 text-2xl font-semibold', isPending ? 'text-amber-200' : 'text-emerald-200')}>
          ...
          <CardContent className="space-y-5 p-5 sm:p-6">
              ...
              <div className="rounded-3xl border border-white/[0.06] bg-gradient-to-br from-white/[0.04] via-white/[0.02] to-gold-500/5 p-5 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1 space-y-2">
                    <p className="text-[10px] uppercase tracking-[0.32em] text-muted-foreground">Country Flag</p>
                    <h2 className="text-2xl font-semibold text-foreground sm:text-3xl">{countryName}</h2>
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant={isPending ? 'pending' : 'active'}>{isPending ? 'Waiting for assignment' : 'Assigned delegate country'}</Badge>
                      <Badge variant="default">{session?.paymentMethod ? session.paymentMethod.replace('_', ' ') : 'delegate profile'}</Badge>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  ...
                  <AllocationField label="Allocation Date" value={allocationDate ? formatDate(allocationDate) : 'Pending'} icon={CalendarDays} />
                </div>
              </div>
              ...
              <div className="grid gap-4">
                <div className="rounded-3xl border border-white/[0.06] bg-white/[0.02] p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.28em] text-muted-foreground">Country Name</p>
                ...
                <div className="rounded-3xl border border-white/[0.06] bg-white/[0.02] p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.28em] text-muted-foreground">Agenda</p>
                ...
                <div className="rounded-3xl border border-white/[0.06] bg-white/[0.02] p-5">
                  <div className="flex items-center gap-3">
                    <div className={cn('flex h-11 w-11 items-center justify-center rounded-2xl border', isPending ? 'border-amber-500/20 bg-amber-500/10 text-amber-400' : 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400')}>
                      {isPending ? <AlertCircle className="h-5 w-5" /> : <Users className="h-5 w-5" />}
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.28em] text-muted-foreground">Status</p>
                      <p className="mt-1 text-lg font-medium text-foreground">{isPending ? 'Pending allocation review' : 'Allocation confirmed'}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                ...
                <p className="mt-3 text-sm text-foreground">{allocatedEvent?.name || 'Event profile pending'}</p>
              </div>
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.28em] text-muted-foreground">
                ...
                <p className="mt-3 text-sm text-foreground">{allocatedCommittee ? `${formatNumber(allocatedCommittee.delegates ?? 0)} / ${formatNumber(allocatedCommittee.capacity ?? 0)}` : 'Pending assignment'}</p>
              </div>
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.28em] text-muted-foreground">
                ...
                <p className="mt-3 text-sm text-foreground">{isPending ? 'Waiting on committee allocation' : 'Allocation live'}</p>
              </div>
            </div>
            ...
            {isPending && (
              <div className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-amber-500/20 bg-amber-500/10 p-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-amber-100/75">Your country allocation will appear here once the delegate assignment is confirmed.</p>
                </div>
                <Button size="sm" variant="outline" asChild>
